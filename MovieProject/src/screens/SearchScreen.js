...
const scrollToTop=()=>{
  ...
  const getMovies = () => {
    if (query !== '') {
      axios
        .get(
          'https://api.themoviedb.org/3/search/movie?api_key=97d61083fe94bbb0ca0fa483cc1b6041&language=en-US&query=' +
            query +
            '&page=' +
            currentPage +
            '&include_adult=false',
    ...
    <Text style={{color:'white'}}>Take me to the up</Text>
      ...
      {movieList.length === 0 ? (
        <View style={styles.couldntFindContainer}>
          <Text style={[styles.couldntFind, {color: theme.fontColor}]}>
        ...
        <FlatList
          ...
          ListFooterComponent={
            currentPage <= lastPage ? (
              ...
              <Text style={styles.endOfListTextStyle}>End of the list</Text>
            )
          }
